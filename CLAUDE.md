...
- **Finished code change = commit + push to origin.** Railway auto-deploys on push. Never leave a genuinely finished, shippable change sitting as a local-only commit.
- **But do not push on every intermediate step of a multi-step task.** Railway rebuilds and restarts the container on every push to `main` regardless of what changed, so pushing after each small revision (a research file, a doc edit, each rewrite of a one-off diagnostic script while iterating on it) burns a full deploy cycle per push for zero user-facing benefit until the whole task is actually done. User feedback 2026-08-21, after a diagnostic script got pushed 5 times across one iterative session: "That's wasting our Railway credits. Push everything when we are done." Commit locally at every step (protects the work, costs nothing), but hold the push until the complete deliverable is ready, then push once. The only exception: a one-off script needs to physically run on Railway's console to return data with no other way to get it, in which case get it as close to final as possible first, then push once, not after every tweak. A one-time fix exists but is not set up: Railway's per-service Watch Paths setting can exclude doc-only paths (`research/**`, `*.md`) from triggering a deploy at all, but it does not help for code/scripts, which need the batching discipline regardless.
