...
- **NEVER `git push` without an explicit go-ahead from the user, given after the work is staged and described.** This is a hard gate, not a courtesy. Commit locally as often as needed (cheap, protects work), but every push, no matter how finished or "obviously ready" the work looks, waits for the user to actually say to push it. Violated 2026-08-21: after being told to batch pushes ("push everything when we are done"), I read that as standing authorization and pushed a completed article without asking, live to Railway, before the user had approved it. Correction, verbatim: "You can't push things before I tell you to." Batching (below) and authorization (this bullet) are two separate rules; satisfying one does not satisfy the other.
- **Do not push on every intermediate step of a multi-step task, once you do have the go-ahead to ship.** Railway rebuilds and restarts the container on every push to `main` regardless of what changed, so pushing after each small revision (a research file, a doc edit, each rewrite of a one-off diagnostic script while iterating on it) burns a full deploy cycle per push for zero user-facing benefit until the whole task is actually done. Commit locally at every step, hold the push until the complete deliverable is ready AND approved, then push once. The only exception: a one-off script needs to physically run on Railway's console to return data with no other way to get it, in which case get it as close to final as possible first, then push once (still fine to do without asking, since nothing user-facing ships), not after every tweak. A one-time fix exists but is not set up: Railway's per-service Watch Paths setting can exclude doc-only paths (`research/**`, `*.md`) from triggering a deploy at all, but it does not help for code/scripts, which need the batching discipline regardless.
...
- **Never generate a blog cover image yourself, via code, Playwright, or otherwise.** Violated 2026-08-21: built a custom SVG/Playwright-composited cover for `youtube-title-length` without asking, and it wasn't even a photo, breaking the site's photographic house style on top of the authorization problem. User correction: "who told you to design that cover image? We do our cover images in a different way." The actual process (confirmed against the sister `niche_website` project, which uses the same pattern): Claude writes a descriptive text prompt (the scene, matching this site's "creator mid-task, warm light, candid, over-the-shoulder" style, plus the exact required output size, verified against an existing real cover file's dimensions via `sharp().metadata()`, never assumed), the user runs that prompt through their own image tool and drops the result in, Claude resizes/crops to spec (1600x900, convert PNG source to JPG at ~100-150KB, delete the PNG after). This applies specifically to **cover images** (the photographic hero image). Data visualizations (charts, graphs) for data-study articles are a different category and fine to build directly, once actually requested, not assumed, follow the `dataviz` skill and this site's own type/color tokens (Fraunces/Barlow, the `--yte-*` custom properties in `frontend/src/blog/proseStyles.jsx`) rather than inventing a new visual system. When building any inline SVG/HTML chart image locally: fonts loaded via `@font-face` with `file:///` paths can silently fail over Chromium's `file://` origin even when the path is correct — embed fonts as base64 data URIs instead, and verify with `document.fonts.check()` programmatically rather than trusting the visual render (this exact bug shipped once before being caught).
...
**Exception for data studies:** "creative" does not mean clever wordplay. `youtube-title-length`'s closing H2 was originally "Write the Title for the Idea, Not the Counter", flagged directly: "Vague subhead... I don't want clever subheads." Replaced with "Stop Optimizing Title Length", plain and declarative, matching every other H2 in that piece (each one already states a finding, e.g. "A Year Tag Is the Only Pattern With a Real Signal"). For a Backlinko-style data study, every heading including the last one should be a clear claim a skimming reader can act on, not a rhetorical flourish. The listicle/guide clusters (video-ideas, etc.) are a different format where a genuinely creative closing line still applies.
