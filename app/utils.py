...
def get_or_create_subscription(db, channel_id: str, email: str = None) -> UserSubscription:
    ...
    if not sub:
        # No free trial (2026-07-26). New rows start at 0 credits (column
        # default) — users must load a pack before anything is billable.
        # Paid activation (billing._activate) overrides allowance / used /
        # reset_date explicitly, so seeding free here is safe for the
        # upgrade path too.
