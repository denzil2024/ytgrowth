/* Single source of truth for YouTube earnings math.
   Used by the money calculator widget (components/MoneyCalculatorWidget.jsx)
   AND the per-niche earnings pages (/youtube-earnings/:niche). Keeping the
   RPM table here means the embedded calculator and the page's headline stat
   cards can never show contradicting numbers.

   RPM = what the creator keeps per 1,000 views, after YouTube's 45% cut and
   after the views that never saw an ad. CPM (advertiser gross) is derived
   from RPM at the standard 55% creator ad-share, so the two stay in lockstep.

   niche keys match CATEGORY_META ids in youtubeStatsCategories.js so an
   earnings page can look its RPM up by the same slug used for /youtube-stats. */

export const NICHES = [
  { key: 'finance',       label: 'Finance & Investing',   low: 18, high: 40 },
  { key: 'tech',          label: 'Tech & Reviews',        low: 10, high: 22 },
  { key: 'education',     label: 'Education & Science',   low: 8,  high: 18 },
  { key: 'news',          label: 'News & Politics',       low: 5,  high: 13 },
  { key: 'beauty',        label: 'Beauty & Makeup',       low: 6,  high: 14 },
  { key: 'fitness',       label: 'Fitness & Health',      low: 6,  high: 12 },
  { key: 'travel',        label: 'Travel',                low: 5,  high: 11 },
  { key: 'cooking',       label: 'Cooking & Food',        low: 4,  high: 9  },
  { key: 'vlogs',         label: 'Vlogs & Lifestyle',     low: 4,  high: 9  },
  { key: 'sports',        label: 'Sports',                low: 3,  high: 7  },
  { key: 'gaming',        label: 'Gaming',                low: 2,  high: 6  },
  { key: 'comedy',        label: 'Comedy',                low: 2,  high: 5  },
  { key: 'entertainment', label: 'Entertainment',         low: 2,  high: 5  },
  { key: 'music',         label: 'Music',                 low: 1.5, high: 4 },
]

export const NICHE_LOOKUP = Object.fromEntries(NICHES.map(n => [n.key, n]))
...
export const COUNTRY_LOOKUP = Object.fromEntries(COUNTRIES.map(c => [c.key, c]))

/* CPM (advertiser gross) from RPM at the standard 55% creator ad-share. */
export const AD_SHARE = 0.55
export function rpmToCpm(rpm) { return rpm / AD_SHARE }
...
/* Country splits to render on a niche page, keyed by niche. Only the niches
   where Search Console showed real geo-specific demand get a split block. */
export const NICHE_COUNTRY_SPLITS = {
  finance: ['tier1', 'tier6'],   // "finance channel rpm in india/usa" demand
...
export const EARNINGS_COUNTRIES = [
  ...
  { key: 'germany',        name: 'Germany',           short: 'Germany',    demonym: 'German',     mult: 0.72,
    tier1: false, cur: { code: 'EUR', sym: '€',  perUsd: 0.92,   locale: 'de-DE' },
    note: 'the strongest ad market in continental Europe, where advertiser spend runs well above the global average but a little below the US',
    payNote: 'AdSense pays in US dollars and your German bank converts to euros, so the € total shifts slightly with the exchange rate. Payout clears once you pass the $100 threshold.',
    taxNote: 'YouTube income is self-employment income in Germany, subject to income tax and, once you pass the Kleinunternehmer threshold, VAT, so most creators register with the Finanzamt and keep proper invoices.' },
  { key: 'ukraine',        name: 'Ukraine',           short: 'Ukraine',    demonym: 'Ukrainian', mult: 0.22, article: 'a',
    tier1: false, cur: { code: 'UAH', sym: '₴',  perUsd: 41,     locale: 'uk-UA' },
    note: 'an engaged Eastern European market where local advertiser spend keeps per-view rates well below Western Europe',
    payNote: 'AdSense pays in US dollars and your Ukrainian bank converts to hryvnia, so the ₴ total tracks the USD/UAH rate. Payout clears after the $100 threshold.',
    taxNote: 'YouTube income is taxable in Ukraine, and many creators register as a sole proprietor (ФОП) on the simplified tax system, which is a common low-rate route for online earnings.' },
  { key: 'philippines',    name: 'the Philippines',   short: 'Philippines', demonym: 'Filipino',  mult: 0.18,
    tier1: false, cur: { code: 'PHP', sym: '₱',  perUsd: 57,     locale: 'en-PH' },
    note: 'a huge, highly-engaged English-speaking audience where low local ad spend keeps per-view rates far below tier-1 countries',
    payNote: 'AdSense pays in US dollars and your Philippine bank converts to pesos, so the ₱ total depends on the USD/PHP rate. Payout clears after the $100 threshold.',
    taxNote: 'YouTube income is taxable in the Philippines as self-employment / professional income, registered with the BIR, with percentage or income tax due depending on your earnings bracket.' },
  { key: 'indonesia',      name: 'Indonesia',         short: 'Indonesia',  demonym: 'Indonesian', mult: 0.16,
    tier1: false, cur: { code: 'IDR', sym: 'Rp', perUsd: 16200,  locale: 'id-ID' },
    note: 'one of the largest audiences on YouTube, where enormous view volume offsets a low local ad rate',
    payNote: 'AdSense pays in US dollars and your Indonesian bank converts to rupiah, so the Rp total tracks the USD/IDR rate. Payout clears after the $100 threshold.',
    taxNote: 'YouTube income is taxable in Indonesia as business income and is reported on your annual SPT, so keeping records of earnings and expenses matters once the channel scales.' },
  { key: 'nigeria',        name: 'Nigeria',           short: 'Nigeria',    demonym: 'Nigerian',   mult: 0.14,
    tier1: false, cur: { code: 'NGN', sym: '₦',  perUsd: 1550,   locale: 'en-NG' },
    note: 'Africa’s biggest creator market, where a large audience and low local ad spend mean earnings come from volume rather than rate',
    payNote: 'AdSense pays in US dollars and your Nigerian bank converts to naira, so the ₦ total swings with the USD/NGN rate. Payout clears after the $100 threshold.',
    taxNote: 'YouTube income is taxable in Nigeria as self-employment income, and creators earning at scale typically register with the FIRS and file annually.' },
]
...
/* Per-niche editorial copy for the /youtube-earnings/:niche pages. Hand-written
   ...
   table above (the embedded calculator reads the same source). Niches without
   an entry fall back to generated-from-data copy on the page. */
export const EARNINGS_META = {
  finance: {
    ...
    whyHeading: 'Why finance RPM is so high',
    why: [
  ...
  cooking: {
    ...
    whyHeading: 'Why cooking RPM is mid-tier',
    why: [
    ...
    faqs: [
      ...
      { q: 'Do cooking YouTubers earn more from ads or sponsorships?',
        a: 'Sponsorships and products, for almost every established food channel. Cookware, knives, appliances, and meal-kit services pay well for integrations, and the top creators launch cookbooks, sauces, and kitchen lines that become the largest income stream over time. Ads are a steady floor; the brand and product side is the ceiling.' },
      { q: 'How many views does a cooking channel need to make $5,000 a month?',
        a: 'At the middle of the cooking RPM band (around $6 to $7 for a tier-1 audience), roughly 750,000 to 850,000 monthly views gets you to $5,000 a month from ads. Sponsorships from cookware and meal-kit brands can get a smaller, engaged food channel to that income at well under half a million views.' },
      { q: 'How can a cooking channel increase its RPM?',
        a: 'Push videos past 8 minutes (technique deep-dives and method explainers justify the length) so they qualify for mid-roll ads. Lean toward higher-value sub-topics like kitchen-gear reviews and restaurant-grade technique over quick recipe clips. And build a tier-1 audience share with globally-framed dishes rather than only region-locked content. Each of these lifts the rate on the views you already have.' },
    ],
  },
  ...
  gaming: {
    ...
    whyHeading: 'Why gaming RPM is low',
    why: [
    ...
    faqs: [
      { q: 'How much do gaming YouTubers make per 1,000 views?',
      ...
      { q: 'How much does a gaming channel make per million views?',
        a: 'At a tier-1 RPM of $2 to $6, one million monthly views works out to roughly $2,000 to $6,000 per month from ads alone. The largest gaming channels clear far more by sheer volume and by stacking peripheral, energy-drink, and game-launch sponsorships that often dwarf the ad revenue.' },
      { q: 'Is gaming a high-CPM niche on YouTube?',
        a: 'No, gaming is one of the lowest-CPM major niches, alongside comedy, entertainment, and music. The young, mobile, global audience and lower-margin advertisers keep the ad rate down. Gaming earns through scale, watch time, and sponsorships rather than a high per-view rate.' },
      { q: 'Why is gaming RPM so low compared to finance?',
        a: 'A bank or software company earns hundreds to thousands per acquired customer and will outbid a game or energy-drink brand many times over for the same slot. A finance viewer is simply worth far more to advertisers than a gaming viewer, so the auction passes a much higher rate to finance creators.' },
      { q: 'How much does audience country change gaming earnings?',
        a: 'Significantly. The $2 to $6 band is for a tier-1 audience; a tier-3 or heavily global gaming audience can earn a third of that or less per view. Gaming audiences are very international, so many large gaming channels run a lower effective RPM than the headline tier-1 figure suggests.' },
      { q: 'How many views does a gaming channel need to make $5,000 a month?',
        a: 'At the middle of the gaming RPM band (around $4 for a tier-1 audience), roughly 1.25 million monthly views gets you to $5,000 a month from ads. Most full-time gaming creators reach that income at fewer views by adding sponsorships, memberships, and Super Chat from streams.' },
      { q: 'Do gaming YouTubers earn more from ads or sponsorships?',
        a: 'Sponsorships, for almost every full-time gaming channel. Peripheral brands, energy drinks, VPNs, and game publishers pay well for integrations, often several times a channel’s ad revenue. Live-stream income (memberships, Super Chat, Super Stickers) is a major third line on top.' },
      { q: 'How can a gaming channel increase its RPM?',
        a: 'Lean into long-form (20+ minute Let’s Plays and reviews) so each view carries multiple mid-rolls, the single biggest lever in a low-rate niche. Build a tier-1 audience share where you can, and pursue sponsorships and memberships rather than relying on the ad rate, which is structurally low in gaming.' },
    ],
  },
  ...
  tech: {
    ...
    whyHeading: 'Why tech RPM is so high',
    why: [
    ...
    faqs: [
      { q: 'How much do tech YouTubers make per 1,000 views?',
        a: "A tech or review channel with a tier-1 (US, UK, Canada, Australia) audience typically earns an RPM of $10 to $22 per 1,000 views, two to three times the YouTube average. RPM is what you keep after YouTube's 45% cut and unmonetized views. Software-adjacent and B2B tech (productivity, cloud, AI tooling) lands at the top of that band." },
      { q: 'How much does a tech channel make per million views?',
        a: 'At a tier-1 RPM of $10 to $22, one million monthly views works out to roughly $10,000 to $22,000 per month from ads alone. For most established tech channels that is the smaller half of the picture: brand sponsorships and affiliate income on gear typically add more on top.' },
      { q: 'Is tech a high-CPM niche on YouTube?',
        a: 'Yes, tech is second only to finance among the major niches. The audience is affluent and concentrated in tier-1 countries, and the topics overlap with expensive advertiser categories like software and gadgets, so the ad rate runs well above the platform average.' },
      { q: 'Why is tech RPM so high?',
        a: 'Because the audience and the topics are both premium. Tech viewers are early adopters with above-average income, and the products advertised (software, hardware, financial tools) are high-value, so advertisers compete hard and pay a premium to reach them.' },
      { q: 'How much does audience country change tech earnings?',
        a: 'A lot. The $10 to $22 band is for a tier-1 audience; a tier-3 or global tech audience earns a fraction of that for the same video. Because so much of tech is in English and globally relevant, building a US and UK audience share is one of the biggest earnings levers in the niche.' },
      { q: 'How many views does a tech channel need to make $5,000 a month?',
        a: 'At the middle of the tech RPM band (around $16 for a tier-1 audience), roughly 310,000 monthly views gets you to $5,000 a month from ads. With sponsorships and gear affiliates, many tech channels hit that income at well under 200,000 views.' },
      { q: 'Do tech YouTubers earn more from ads or sponsorships?',
        a: 'Sponsorships, for most full-time tech channels. Software, hardware, VPN, and hosting brands pay heavily for integrations, and affiliate commission on reviewed gear adds a steady stream. Ads are a strong floor in tech, but brands and affiliates are usually the larger lines.' },
      { q: 'How can a tech channel increase its RPM?',
        a: 'Shift toward software and B2B topics (productivity, cloud, security, AI tooling), which carry the highest tech CPMs. Keep videos over 8 minutes for mid-rolls, and build a tier-1 audience share. The B2B corners of tech can clear $30 RPM during budget season, well above consumer-gadget content.' },
    ],
  },
  ...
  fitness: {
    ...
    whyHeading: 'Why fitness RPM is above average',
    why: [
    ...
    faqs: [
      { q: 'How much do fitness YouTubers make per 1,000 views?',
        a: "A fitness or health channel with a tier-1 (US, UK, Canada, Australia) audience typically earns an RPM of $6 to $12 per 1,000 views, above the YouTube average. RPM is what you keep after YouTube's 45% cut and unmonetized views. Education and technique content with an older audience lands higher than quick workout clips." },
      { q: 'How much does a fitness channel make per million views?',
        a: 'At a tier-1 RPM of $6 to $12, one million monthly views works out to roughly $6,000 to $12,000 per month from ads alone. Coaching sales, programming, and supplement or apparel deals usually add more on top, often more than the ads for established creators.' },
      { q: 'Is fitness a high-CPM niche on YouTube?',
        a: 'It is above average but not top-tier. Fitness sits below finance, tech, and education, and above gaming, comedy, and music. Health-focused advertisers pay a premium for an engaged audience, but the rate does not reach finance or software levels.' },
      { q: 'Why is fitness RPM above average?',
        a: 'Because supplement, apparel, and fitness-app advertisers value a committed health audience and bid above the platform norm for it. The viewer is seen as a strong prospect for recurring products like supplements and app subscriptions, which lifts the rate.' },
      { q: 'How much does audience country change fitness earnings?',
        a: 'Considerably. The $6 to $12 band is for a tier-1 audience; a tier-3 or global audience earns a fraction of that for the same video. Follow-along workouts travel well internationally, so many fitness channels run a lower effective rate than the tier-1 figure unless they skew toward US and UK viewers.' },
      { q: 'How many views does a fitness channel need to make $5,000 a month?',
        a: 'At the middle of the fitness RPM band (around $9 for a tier-1 audience), roughly 560,000 monthly views gets you to $5,000 a month from ads. With coaching sales and supplement or apparel deals, many fitness channels reach that income at far fewer views.' },
      { q: 'Do fitness YouTubers earn more from ads or coaching and products?',
        a: 'Coaching and products, for most established fitness channels. Programming sales, app subscriptions, supplement and apparel lines, and one-on-one coaching usually out-earn ads. The channel is the funnel; the products are where the income concentrates.' },
      { q: 'How can a fitness channel increase its RPM?',
        a: 'Mix in education and technique videos (which carry an older, higher-value audience) alongside follow-along workouts, keep videos over 8 minutes for mid-rolls, and build a tier-1 audience share. Then layer coaching and product offers, which is where fitness income really scales.' },
    ],
  },

  beauty: {
    intro: [
      "Beauty and makeup is an above-average earner on YouTube. A tier-1 audience pulls an RPM of about $6 to $14 per 1,000 views. Luxury and skincare brands bid well for a beauty audience, and channels covering high-end products see the upper end of that range.",
      "But ads are rarely the main line. The biggest beauty creators earn most of their income from brand partnerships, affiliate commission on products, and their own makeup or skincare lines, which over time dwarf AdSense.",
    ],
    whyHeading: 'Why beauty RPM runs above average',
    why: [
      "Brand demand is strong. Luxury skincare and cosmetics advertisers bid well for a beauty audience, and the higher-end the products covered, the higher the rate, toward $14.",
      "Image quality matters here. Beauty is one of the few niches where production quality directly affects credibility and watch time, which in turn feeds the algorithm and the rate.",
      "Products and partnerships dominate. Affiliate commission (Sephora, Ulta, Amazon), brand deals, and owned product lines are where top beauty creators earn 70 to 90% of their income.",
    ],
    faqs: [
      { q: 'How much do beauty YouTubers make per 1,000 views?',
        a: "A beauty or makeup channel with a tier-1 (US, UK, Canada, Australia) audience typically earns an RPM of $6 to $14 per 1,000 views, above the YouTube average. RPM is what you keep after YouTube's 45% cut and unmonetized views. Luxury skincare and high-end product content lands at the top of that band; drugstore hauls land lower." },
      { q: 'How much does a beauty channel make per million views?',
        a: 'At a tier-1 RPM of $6 to $14, one million monthly views works out to roughly $6,000 to $14,000 per month from ads alone. For established beauty creators that is usually the smaller part: affiliate commission, brand deals, and owned product lines typically earn several times the ad revenue.' },
      { q: 'Is beauty a high-CPM niche on YouTube?',
        a: 'It is above average, lifted by luxury and skincare advertisers, but not at finance or tech levels. Channels covering high-end products see stronger rates than those covering budget or drugstore items, because the brands behind premium beauty pay more for the audience.' },
      { q: 'Why does beauty RPM run above average?',
        a: 'Because luxury skincare and cosmetics brands compete for an engaged, purchase-minded audience and bid above the platform norm. The more premium the products a channel covers, the more advertisers are willing to pay to appear alongside it.' },
      { q: 'How much does audience country change beauty earnings?',
        a: 'A lot. The $6 to $14 band is for a tier-1 audience; a tier-3 or global beauty audience earns a fraction of that for the same video. Beauty content travels widely, so channels with a heavily international audience run a lower effective rate than the tier-1 figure unless they skew toward US and UK viewers.' },
      { q: 'How many views does a beauty channel need to make $5,000 a month?',
        a: 'At the middle of the beauty RPM band (around $10 for a tier-1 audience), roughly 500,000 monthly views gets you to $5,000 a month from ads. With affiliate links and brand deals, many beauty channels reach that income at well under half a million views.' },
      { q: 'Do beauty YouTubers earn more from ads or products and affiliates?',
        a: 'Products and affiliates, by a wide margin for top channels. Affiliate commission, brand partnerships, and owned makeup or skincare lines make up 70 to 90% of income for the biggest beauty creators. Ads are a steady base, not the main event.' },
      { q: 'How can a beauty channel increase its RPM?',
        a: 'Cover higher-end products (luxury skincare and premium cosmetics pull stronger ad rates than drugstore content), keep videos over 8 minutes for mid-rolls, and build a tier-1 audience share. Then lean into affiliate links and brand partnerships, which is where beauty income concentrates.' },
    ],
  },
}
