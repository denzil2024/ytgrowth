    ...
    <div style={{ marginBottom: 16, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
      <button onClick={() => setOpen(v => !v)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: C.purple }}>Viral Format Templates</span>
          <span style={{ fontSize: 11, color: C.text3, fontWeight: 400 }}>click any to pre-fill your title</span>
        </div>
        ...
        <div style={{ padding: '12px 14px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, background: C.card }}>
          {VIRAL_FORMATS.map(fmt => (
            <div key={fmt.key} onClick={() => onUse(fmt.example)}
              ...
              <p style={{ fontSize: 11, fontWeight: 700, color: C.purple, marginBottom: 3 }}>{fmt.label}</p>
              <p style={{ fontSize: 12, color: C.text1, fontWeight: 500, lineHeight: 1.35, marginBottom: 3 }}>{fmt.example}</p>
              <p style={{ fontSize: 10.5, color: C.text3 }}>{fmt.why}</p>
            </div>
          ))}
        </div>
      )}
    ...
    <div style={{ maxWidth: 860, fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: '22px 24px', marginBottom: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
        ...
        <button onClick={handleSubmitTitle} disabled={loading || loadingIntent || !title.trim()}
          style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 22px', background: title.trim() && !loading && !loadingIntent ? C.blue : '#c7c7cc', color: '#fff', border: 'none', borderRadius: 9, fontSize: 13.5, fontWeight: 700, fontFamily: 'inherit', cursor: title.trim() && !loading && !loadingIntent ? 'pointer' : 'not-allowed', transition: 'background 0.15s' }}>
          {loadingIntent ? (
            <>
      ...
      {intentOptions && !loading && !result && (
        <div style={{ background: C.card, border: `1.5px solid ${C.blue}`, borderRadius: 16, padding: '22px 24px', marginBottom: 20, boxShadow: '0 2px 12px rgba(0,122,255,0.08)' }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: C.text1, marginBottom: 4 }}>What's this video really about?</p>
              ...
              <div key={i} onClick={() => handleSelectIntent(opt.keyword)}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                    <span style={{ fontSize: 12.5, fontWeight: 700, color: C.text1 }}>{opt.label}</span>
                    <span style={{ fontSize: 10, fontWeight: 700, color: C.blue, background: C.blueBg, padding: '1px 7px', borderRadius: 20 }}>{opt.keyword}</span>
                  </div>
          ...
          <button onClick={() => runAnalysis('')}
            style={{ marginTop: 12, fontSize: 11.5, color: C.text3, background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', fontFamily: 'inherit' }}>
        ...
        <>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {result.primary_phrase && (
                    <div style={{ marginTop: 8 }}>
                      <span style={{ fontSize: 11, color: C.text3 }}>Niche keyword: </span>
                  ...
                  {result.viral_format_detected && (
                    <span style={{ display: 'inline-block', marginTop: 6, fontSize: 10, fontWeight: 700, background: C.purpleBg, color: C.purple, padding: '3px 8px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
          ...
          {result.keyword_scores?.length > 0 && (
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: '18px 24px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: C.text3, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 3 }}>Keyword Research</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0, marginBottom: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 60px 85px 50px', gap: 8, padding: '6px 10px', background: '#f5f5f7', borderRadius: '8px 8px 0 0', borderBottom: `1px solid ${C.border}` }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: C.text3, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Keyword phrase</span>
                  <span style={{ fontSize: 10, fontWeight: 700, color: C.text3, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Volume</span>
                  <span style={{ fontSize: 10, fontWeight: 700, color: C.text3, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Competition</span>
                  <span style={{ fontSize: 10, fontWeight: 700, color: C.text3, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Score</span>
                </div>
                {result.keyword_scores.map((kw, i) => {
                  ...
                  return (
                    <div key={kw.phrase} style={{ display: 'grid', gridTemplateColumns: '1fr 60px 85px 50px', gap: 8, padding: '9px 10px', borderBottom: i < result.keyword_scores.length - 1 ? `1px solid ${C.border}` : 'none', background: i % 2 === 0 ? '#fff' : '#fafafc' }}>
                      <span style={{ fontSize: 12.5, fontWeight: 600, color: C.text1 }}>{kw.phrase}</span>
                      <span style={{ fontSize: 11, fontWeight: 700, color: volColor }}>{kw.volume}</span>
                      <span style={{ fontSize: 11, fontWeight: 700, color: compColor }}>{kw.competition}</span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {fixes.map(([key, meta]) => (
                    <div key={key} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: C.orange, minWidth: 100, paddingTop: 1 }}>{meta.label}</span>
          ...
          {result.intent_analysis?.search_intent && (
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: '20px 24px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: C.text3, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 14 }}>Search Intent Analysis</p>
                  ...
                  return (
                    <div key={i} style={{ border: `1.5px solid ${copied === i ? '#bbf7d0' : C.border}`, borderRadius: 12, overflow: 'hidden', background: copied === i ? C.greenBg : C.card }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 14px', background: hm.bg, borderBottom: `1px solid ${C.border}` }}>
                        <span style={{ fontSize: 10, fontWeight: 800, color: hm.color, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{hm.label}</span>
                      ...
                      <div onClick={() => copyTitle(s.title, i)} style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, padding: '12px 14px', cursor: 'pointer' }}>
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5, flexWrap: 'wrap' }}>
                            ...
                            {s.power_words_found?.length > 0 && s.power_words_found.map(w => (
                              <span key={w} style={{ fontSize: 10, fontWeight: 700, background: C.blueBg, color: C.blue, padding: '1px 6px', borderRadius: 20, textTransform: 'uppercase' }}>{w}</span>
                            ))}
                          </div>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
                          ...
                          <span style={{ fontSize: 11, color: copied === i ? C.green : C.blue, fontWeight: 600 }}>{copied === i ? '✓ Copied' : 'Copy'}</span>
                        </div>
                      </div>
                      {/* Why it works */}
                      {s.why_it_works && (
                        <div style={{ padding: '8px 14px 12px', borderTop: `1px solid ${C.border}`, background: '#fafafc' }}>
                          <p style={{ fontSize: 11.5, color: C.text2, lineHeight: 1.55 }}>
          ...
          {result.top_videos?.length > 0 && (
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: '22px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: C.text3, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>Top titles matching your topic</p>
              <p style={{ fontSize: 12, color: C.text3, marginBottom: 16 }}>
