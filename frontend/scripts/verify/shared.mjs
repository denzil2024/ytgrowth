/* Shared gated/onboarding components harness. Usage: node scripts/verify/shared.mjs */
...
try{
  ...
  await pg.goto(`http://localhost:${PORT}/preview-shared.html`,{waitUntil:'networkidle'})
  await pg.evaluate(()=>document.fonts?document.fonts.ready:Promise.resolve())
  await pg.waitForTimeout(1800)
  let out=path.join(__dirname,'shared-stack.png')
  await pg.screenshot({path:out,fullPage:true})
  console.log(`OK: wrote ${out}`)
  const pg2=await ctx.newPage()
  await pg2.goto(`http://localhost:${PORT}/preview-shared.html?modal=1`,{waitUntil:'networkidle'})
  await pg2.evaluate(()=>document.fonts?document.fonts.ready:Promise.resolve())
  await pg2.waitForTimeout(1200)
  out=path.join(__dirname,'shared-modal.png')
  await pg2.screenshot({path:out,fullPage:false})
  console.log(`OK: wrote ${out}`)
  await ctx.close()
