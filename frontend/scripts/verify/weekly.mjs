/*
 * Weekly Report preview screenshot harness. Mirrors outliers.mjs.
 * Spawns vite dev, loads each preview state, captures screenshots.
 *
...
try {
  const states = ['reports', 'empty', 'free']
  for (const state of states) {
    const vp = state === 'reports' ? { width: 1440, height: 1700 } : VIEWPORT
    const ctx = await browser.newContext({ viewport: vp, deviceScaleFactor: 1 })
    const page = await ctx.newPage()
    await page.goto(`http://localhost:${PORT}/preview-weekly.html?state=${state}`, { waitUntil: 'networkidle' })
    await page.evaluate(() => document.fonts ? document.fonts.ready : Promise.resolve())
    await page.waitForTimeout(900)
    const out = path.join(__dirname, `weekly-${state}.png`)
