/*
 * AuditProgress preview screenshot harness. Spawns vite dev, loads the
 * running + done states in the faithful Overview shell, captures screenshots.
 *
 * Usage:  node scripts/verify/auditprogress.mjs
 */
...
try {
  // running: wait ~7s so stage 1 completes and stage 2 is active.
  for (const [state, settle] of [['running', 7000], ['done', 1200]]) {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 900 }, deviceScaleFactor: 1.25 })
    const page = await ctx.newPage()
    await page.goto(`http://localhost:${PORT}/preview-auditprogress.html?state=${state}`, { waitUntil: 'networkidle' })
    await page.evaluate(() => document.fonts ? document.fonts.ready : Promise.resolve())
    await page.waitForTimeout(settle)
    const out = path.join(__dirname, `auditprogress-${state}.png`)
